...
### `GET /api/controlpanel/wallet-issues` — Verificar por carteira

Backing do modal **"Verificar por carteira"**, aberto ao **clicar no nome da
empresa** (primeira coluna EMPRESA). Permite ao operador escolher uma carteira e
ver **quais issues** estão relacionadas a ela na **data selecionada** em tela.

| Param     | Obrigatório | Descrição |
|-----------|-------------|-----------|
| companyId | sim         | Confrontado com `company_visible` (403 se invisível). |
| date      | sim         | `YYYY-MM-DD`. |
| walletId  | não         | Ausente → **modo lista** (seletor); presente → **modo issues** (detalhe). |

**Fonte única:** o pre-processing (E), via `beehus_catalog.issues_by_wallet_detail`
— **uma** chamada por `(company, date)` que varre os 6 tipos de issue e agrupa os
`*Detailed` por `walletId` (expandindo `affectedWallets`). Como
`get_preprocessing_status` **não é cacheado**, agregar os 6 tipos aqui evita 6
chamadas à mesma rota. São os **mesmos arrays** que o grid (`/rows`) e o `/detail`
consomem, então a contagem por carteira nunca diverge do grid.

- **Modo lista** (sem `walletId`) — todas as carteiras da empresa
  (`wallets_for_company`) com o contador de issues pendentes:
  ```json
  { "companyId": "...", "companyName": "...", "date": "YYYY-MM-DD",
    "totalWithIssues": 3,
    "wallets": [ {"walletId": "...", "walletName": "...", "issueCount": 2} ] }
  ```
  Ordenação: carteiras **com issue primeiro** (mais issues no topo), depois
  alfabética. Carteiras que só aparecem nas issues (fora do índice
  `partner_wallets`) são anexadas defensivamente para não ficarem órfãs.

- **Modo issues** (com `walletId`) — issues daquela carteira agrupadas pelos 6
  tipos do Painel em ordem canônica (`ISSUE_TYPES`), cada issue enriquecida com
  `walletName`/`beehusName`/`mainId` (mesma regra do `/detail`, via
  `_enrich_issue_securities`):
  ```json
  { "companyId": "...", "companyName": "...", "date": "YYYY-MM-DD",
    "walletId": "...", "walletName": "...", "total": 4,
    "types": [ {"type": "security_unmapped", "label": "Mapeamento",
                "issues": [ {"externalId": "...", "securityId": "...",
                             "beehusName": "...", "mainId": "...",
                             "unprocessedSecurityId": "...", "createdAt": "..."} ]} ] }
  ```

**Frontend** (`templates/controlpanel.html`): a célula do nome da empresa
(`_companyCellHtml`, classe `js-open-wallet-issues`) abre `#wallet-issues-modal`
via `openWalletIssues`. O modal tem duas views numa só janela — o **seletor**
(`#wi-picker`, filtro + lista com badges de contagem) e o **detalhe**
(`#wi-issues`, issues agrupadas por tipo); "← Voltar" (`_wiShowPicker`) alterna
entre elas. Escolher uma carteira chama `loadWalletIssues` → `_wiRenderIssues`.
...
- Backend: [pages/controlpanel.py](../pages/controlpanel.py)
  ...
  `security_type_fields`). Verificar por carteira: `wallet_issues` +
  `_enrich_issue_securities`, apoiado em
  `beehus_catalog.issues_by_wallet_detail` (que reusa `_rows_from_status`,
  extraído de `issues_detail`).
