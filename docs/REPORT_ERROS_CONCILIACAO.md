...
- **`diff.hasTarget`** — existe unprocessed na data-alvo. **Gatilho principal (único).**

> **Caixa-âncora eliminado (jul/2026):** não existe mais `cashStatus`. O caixa **não**
> classifica divergência de ativo. O resíduo de caixa (`diff.cashResidual` = projetado −
> oficial) é **informativo** e vira a linha *"Divergência de caixa"* do relatório quando
> `|resíduo| ≥ 0,01` — sem disparar correção.
...
O motor já se comporta assim: com `hasTarget=false` o diff não é computado — logo
divergências/recon (inclusive a linha *"Divergência de caixa"*, de natureza reconciliação)
**não existem** no Tipo B (não há risco de reportá-los à toa).
...
|---|---|---|
| Transação órfã (sem `securityId`) | `transactions[].securityId` vazio **E** `type` ∈ tipos que EXIGEM securityId (`_SID_REQUIRED_TXN_TYPES`: dividend, cashDividend, interestOnEquity, buySell, coupon, amortization, securityContributionAdjustment, maturity, futuresSettlement, bzFundTax(es), dividendOnboarding, securityTransfer) | **Sim** |
| ~~Transação de carteira sem ativo~~ (NÃO é órfã) | `gainsExpenses`/`rebate`/`withdrawalDeposit`/`taxes`/`contributionAdjustment`/`split`/`inplit` sem `securityId` são LEGÍTIMAS (nível carteira) → **não** sinaliza | — |
...
| **Divergência de caixa** (INFORMATIVA — não dispara correção) | `diff.cashResidual` (caixa **projetado** `diff.movedCash` − **oficial** `diff.officialCash`), reportada quando `|resíduo| ≥ 0,01`. Caixa-âncora eliminado: **não** classifica ativo nem gera recon/escrita — só é visibilizada no relatório. |
...
zerada descartada; alta confiança (1 divergência). Resíduo de caixa **abaixo de 0,01** (ruído
de arredondamento) não vira linha. Sem caixa oficial (forward) não há o que comparar → nada.
