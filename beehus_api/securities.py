...
def get_security(*, security_id: str, timeout: int = 30) -> dict | None:
    """GET /beehus/securities/{securityId} — READ de um ativo específico.

    Devolve o doc do ativo. Para um **ativo de explosão**, o doc traz o campo
    ``correspondingWallet`` (``{"_id", "name", ...}``): a carteira que sofre a
    explosão e que precisa ser co-processada quando a carteira de origem é
    processada (ver `beehus_catalog.explosion_chain`). Retorna ``None`` quando
    ``security_id`` é vazio ou a resposta não é um objeto.
    """
    if not security_id:
        return None
    out = request("GET", f"/beehus/securities/{security_id}", timeout=timeout)
...
def filtered_security_price(
    ...
    **cliente** escolhe o mais específico para o contexto `(company, entity,
    ...
    """
    # IMPORTANTE: `securityIds` deve ir como PARÂMETRO REPETIDO
    # (`?securityIds=a&securityIds=b`), NÃO como CSV (`securityIds=a,b`). O backend
    # devolve VAZIO para o CSV com >1 id (bug confirmado ao vivo: 1 id → N records;
    # `a,b` → 0). O `requests` codifica uma LISTA como parâmetro repetido, então
    # passamos a lista direto. `pricingType` segue como CSV (o backend aceita).
    if isinstance(security_ids, (list, tuple, set)):
        ids = [str(x) for x in security_ids if x]
    else:
        ids = [str(security_ids)] if security_ids else []
    if not ids:
        return []
