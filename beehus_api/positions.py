...
def _get_positions_chunked(path, base_params, wallet_ids, timeout):
    ...
    Blocos em PARALELO (threads; Session tem pool 20): com centenas de carteiras
    o wall-time cai de ~n_blocos×latência p/ ~1×. `ex.map` PRESERVA a ordem de
    concatenação e re-levanta a 1ª exceção na ordem dos blocos — mesma semântica
    do loop sequencial (GETs sem efeito colateral)."""
