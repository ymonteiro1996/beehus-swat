...
# managed_portfolio — "Carteira gerencial" — DISCONTINUED. Its preview/apply
# materialised up to ~12 months of publishedPositionSecurities (≈500k docs,
# ~3s) into memory on a single click — the heaviest DB op in the app — for an
# analysis-only feature with no upstream writes. Removed from _KINDS so it can
# no longer be created/edited; the constant is kept only to gate the dispatch
# points and to hide any already-saved managed exceptions from the listing.
...
def list_exceptions():
    ...
    # Carteira gerencial is discontinued — hide any already-saved
    # managed_portfolio exceptions so the option fully disappears from the UI.
    # The on-disk file is left intact (not deleted), just unlisted/non-actionable.
    items = [it for it in items
             if (it.get("kind") or _KIND_POSITION_STRIP) != _KIND_MANAGED_PORTFOLIO]
...
def preview_exception(exception_id):
    ...
    if kind == _KIND_MANAGED_PORTFOLIO:
        return jsonify({"error": "Carteira gerencial foi descontinuada."}), 410
...
def download_excel(exception_id):
    ...
    # managed_portfolio ("Carteira gerencial") is discontinued — reject here too
    # so an already-saved managed exception can't trigger any of its handlers.
    if kind == _KIND_MANAGED_PORTFOLIO:
        return jsonify({"error": "Carteira gerencial foi descontinuada."}), 410
